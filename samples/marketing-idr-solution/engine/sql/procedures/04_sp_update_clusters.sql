-- ============================================================================
-- SP_UPDATE_CLUSTERS: Incremental connected components
-- Only processes clusters affected by new matches from this pipeline run.
-- Maintains IDR_CORE_CLUSTER_MEMBERSHIP lookup for fast affected-cluster discovery.
-- See SP_UPDATE_CLUSTERS_FULL for the full-recompute fallback.
-- ============================================================================

CREATE OR REPLACE PROCEDURE IDR.PROCEDURES.SP_UPDATE_CLUSTERS(DEPLOYMENT_DB VARCHAR)
RETURNS VARCHAR
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
DECLARE
    ...
    new_edge_count INTEGER DEFAULT 0;
    affected_cluster_count INTEGER DEFAULT 0;
    affected_source_count INTEGER DEFAULT 0;
...
BEGIN
    ...
    -- Determine cutoff: process only match results created since last IDR_COMPLETE event
    LET ts_query VARCHAR := 'SELECT COALESCE(MAX(event_timestamp), ''1970-01-01''::TIMESTAMP_NTZ) AS ts FROM ' ||
        DB || '.SILVER.IDR_CORE_EVENT_LOG WHERE event_type = ''IDR_COMPLETE''';
    LET rs RESULTSET := (EXECUTE IMMEDIATE :ts_query);
    LET cur CURSOR FOR rs;
    FOR row_var IN cur DO
        last_run_ts := row_var.ts;
    END FOR;

    -- Step 1: NEW match edges only (from this run)
    -- v1: skip pairs that have a VETO edge anywhere in MATCH_RESULTS (forward-only).
    EXECUTE IMMEDIATE 'CREATE OR REPLACE TEMPORARY TABLE temp_new_edges AS
    SELECT DISTINCT
    ...
    WHERE is_active = TRUE
      AND created_at > ''' || :last_run_ts || '''::TIMESTAMP_NTZ
    ...
    SELECT COUNT(*) INTO :new_edge_count FROM temp_new_edges;

    -- Step 2: Affected sources (both sides of new edges)
    CREATE OR REPLACE TEMPORARY TABLE temp_affected_sources AS
    SELECT DISTINCT src AS source_record_id FROM (
        SELECT src1 AS src FROM temp_new_edges
        UNION
        SELECT src2 AS src FROM temp_new_edges
    );

    SELECT COUNT(*) INTO :affected_source_count FROM temp_affected_sources;

    -- Step 3: Affected clusters (via CLUSTER_MEMBERSHIP lookup — fast indexed join)
    EXECUTE IMMEDIATE 'CREATE OR REPLACE TEMPORARY TABLE temp_affected_clusters AS
    SELECT DISTINCT cm.cluster_id
    FROM ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP cm
    JOIN temp_affected_sources a ON cm.source_record_id = a.source_record_id';

    SELECT COUNT(*) INTO :affected_cluster_count FROM temp_affected_clusters;

    -- Step 4: Expand to ALL members of affected clusters + orphan sources
    EXECUTE IMMEDIATE 'CREATE OR REPLACE TEMPORARY TABLE temp_affected_all_sources AS
    SELECT source_record_id FROM ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP cm
    WHERE cm.cluster_id IN (SELECT cluster_id FROM temp_affected_clusters)
    UNION
    SELECT source_record_id FROM temp_affected_sources';

    -- Step 5: Subgraph edges (both endpoints in affected set)
    -- v1: same VETO filter applies — skip pairs with any VETO edge.
    ...
    WHERE is_active = TRUE
      AND COALESCE(edge_polarity, ''MATCH'') = ''MATCH''
      AND new_source_record_id IN (SELECT source_record_id FROM temp_affected_all_sources)
      AND matched_source_record_id IN (SELECT source_record_id FROM temp_affected_all_sources)
      AND NOT EXISTS (
        SELECT 1 FROM ' || DB || '.SILVER.IDR_CORE_MATCH_RESULTS v
        WHERE v.is_active = TRUE
          AND v.edge_polarity = ''VETO''
    ...
    -- Fast exit if nothing to do (but still create singletons for orphan source records)
    IF (:new_edge_count = 0) THEN
        EXECUTE IMMEDIATE 'INSERT INTO ' || DB || '.SILVER.IDR_CORE_CLUSTER (cluster_id, source_record_ids, identifier_ids, status)
        SELECT
            UUID_STRING(),
            ARRAY_CONSTRUCT(l.source_record_id),
            ARRAY_AGG(DISTINCT l.identifier_id),
            ''ACTIVE''
        FROM ' || DB || '.SILVER.IDR_CORE_IDENTIFIER_LINK l
        LEFT JOIN ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP cm ON l.source_record_id = cm.source_record_id
        WHERE cm.source_record_id IS NULL AND l.is_active = TRUE
          AND l.created_at > ''' || :last_run_ts || '''::TIMESTAMP_NTZ
        GROUP BY l.source_record_id';
        -- RESULT_SCAN of an INSERT returns a 1-row summary; column $1 is the actual rows-inserted count.
        SELECT $1 INTO :singleton_count FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()));

        IF (:singleton_count > 0) THEN
            EXECUTE IMMEDIATE 'INSERT INTO ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP (source_record_id, cluster_id, updated_at)
            SELECT f.value::VARCHAR, c.cluster_id, CURRENT_TIMESTAMP()
            FROM ' || DB || '.SILVER.IDR_CORE_CLUSTER c,
            LATERAL FLATTEN(input => c.source_record_ids) f
            WHERE c.status = ''ACTIVE''
              AND ARRAY_SIZE(c.source_record_ids) = 1
              AND f.value::VARCHAR NOT IN (SELECT source_record_id FROM ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP)';
        END IF;

        RETURN 'Clusters (INC): ' || :singleton_count || ' singletons created, 0 updated, 0 merges, 0 unchanged. No new edges.';
    END IF;

    -- Step 6: Seeded label propagation
    -- Initialize roots with existing cluster_id (so existing members already share labels).
    -- Only nodes NOT in any cluster get themselves as their own root.
    -- This short-circuits propagation: we only need edges to flip labels when:
    --   (a) a new node joins an existing cluster (picks up the cluster's root), or
    --   (b) a new edge connects two previously separate clusters (MERGE).
    EXECUTE IMMEDIATE 'CREATE OR REPLACE TEMPORARY TABLE roots AS
    SELECT
        s.source_record_id AS node,
        COALESCE(cm.cluster_id, s.source_record_id) AS root
    FROM temp_affected_all_sources s
    LEFT JOIN ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP cm
      ON cm.source_record_id = s.source_record_id';
    ...
    -- Existing mapping LIMITED to affected clusters (not all clusters)
    ...
    WHERE ec.status = ''ACTIVE''
      AND ec.cluster_id IN (SELECT cluster_id FROM temp_affected_clusters)';
    ...
    -- Identifier lookup scoped to subgraph members (much smaller than full identifier_link)
    -- #3: Only pull identifier_link rows for sources in clusters we're actually changing
    ...
    WHERE l.is_active = TRUE
      AND l.source_record_id IN (
          SELECT m.value::VARCHAR FROM temp_cluster_actions ca, LATERAL FLATTEN(input => ca.members) m
          WHERE ca.action IN (''CREATE'', ''UPDATE'', ''MERGE'')
      )';

    -- #3: Only compute identifiers for CREATE/UPDATE/MERGE (UNCHANGED clusters don't need refresh)
    ...
    -- #4: Only flatten members for clusters whose MATCH_LOG will be written
    -- (CREATE with >1 member, any UPDATE/MERGE). Singletons + UNCHANGED skipped.
    ...
    FROM temp_cluster_actions ca,
    LATERAL FLATTEN(input => ca.members) m
    WHERE ca.action IN ('UPDATE', 'MERGE')
       OR (ca.action = 'CREATE' AND ca.member_count > 1);

    -- Match rules: only scan match_results where both sides are in our subgraph.
    -- VETO edges are excluded from audit (they are anti-edges, not "rules that fired").
    ...
    -- ─── Maintain CLUSTER_MEMBERSHIP ────────────────────────────────────
    -- For each processed cluster, replace its members with the new canonical set.
    -- Step A: Delete old membership for all affected clusters + merged clusters
    EXECUTE IMMEDIATE 'DELETE FROM ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP
    WHERE cluster_id IN (
        SELECT existing_cluster_id FROM temp_cluster_actions
        WHERE existing_cluster_id IS NOT NULL AND action IN (''UPDATE'', ''MERGE'')
        UNION
        SELECT old_cluster_id FROM temp_clusters_to_merge
    )
    ...
    -- Step B: Insert fresh membership for CREATE/UPDATE/MERGE actions
    EXECUTE IMMEDIATE 'INSERT INTO ' || DB || '.SILVER.IDR_CORE_CLUSTER_MEMBERSHIP (source_record_id, cluster_id, updated_at)
    SELECT m.value::VARCHAR, 
           CASE WHEN ca.action = ''CREATE'' THEN ca.new_cluster_id ELSE ca.existing_cluster_id END,
           CURRENT_TIMESTAMP()
    FROM temp_cluster_actions ca,
    LATERAL FLATTEN(input => ca.members) m
    ...
    -- Singleton creation: source records from this batch with no cluster membership
    EXECUTE IMMEDIATE 'INSERT INTO ' || DB || '.SILVER.IDR_CORE_CLUSTER (cluster_id, source_record_ids, identifier_ids, status)
    SELECT
        UUID_STRING(),
        ARRAY_CONSTRUCT(l.source_record_id),
    ...
    created_count := :created_count + :singleton_count;

    DROP TABLE IF EXISTS temp_new_edges;
    DROP TABLE IF EXISTS temp_affected_sources;
    DROP TABLE IF EXISTS temp_affected_clusters;
    DROP TABLE IF EXISTS temp_affected_all_sources;
    ...
    RETURN 'Clusters (INC): ' || :created_count || ' created (' || :singleton_count || ' singletons), ' || :updated_count || ' updated, ' || 
           :merge_count || ' merges, ' || :unchanged_count || ' unchanged. Decisions: ' || 
           :match_count || ' MATCH, ' || :no_match_count || ' NO_MATCH' ||
           '. Scope: ' || :new_edge_count || ' new edges, ' || :affected_source_count || ' affected sources, ' ||
           :affected_cluster_count || ' affected clusters';
END;
$$;
