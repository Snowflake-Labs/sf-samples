...
SELECT
	...
	avg(TOTAL_ELAPSED_TIME/10000) as avg_time,
   min(TOTAL_ELAPSED_TIME/10000) as min_time,
   max(TOTAL_ELAPSED_TIME/10000) as max_time,
   
   sum(case when TRANSACTION_BLOCKED_TIME>0 then 1 else 0 end) as lock_cnt,
   avg(TRANSACTION_BLOCKED_TIME/10000) as avg_lock,
   min(TRANSACTION_BLOCKED_TIME/10000) as min_lock,
   max(TRANSACTION_BLOCKED_TIME/10000) as max_lock,
   
   sum(case when TO_NUMBER((QUEUED_PROVISIONING_TIME + QUEUED_OVERLOAD_TIME) / 10000, 10, 3)>0 then 1 else 0 end) as queue_cnt,
   avg(TO_NUMBER((QUEUED_PROVISIONING_TIME + QUEUED_OVERLOAD_TIME) / 10000, 10, 3)) AS avg_QUEUE,
   min(TO_NUMBER((QUEUED_PROVISIONING_TIME + QUEUED_OVERLOAD_TIME) / 10000, 10, 3)) AS min_QUEUE,
   max(TO_NUMBER((QUEUED_PROVISIONING_TIME + QUEUED_OVERLOAD_TIME) / 10000, 10, 3)) AS max_QUEUE
