-------------------------------------------------
-- NAME:	 PERF-AU-81-concurrency-end-all.txt
-------------------------------------------------
-- DESCRIPTION:
...
--JOIN    TABLE($STATEMENT_TYPE) S ON J.STATEMENT_PROPERTIES = S.ID
...
SELECT	START_TIME,
		case when WAREHOUSE_NAME is null then 'GS' else WAREHOUSE_NAME END AS WAREHOUSE_NAME, 
		TIMEADD('SECOND',QUEUED_PROVISIONING_TIME/10000,START_TIME) AS END_TIME
FROM	TABLE($QUERY_HISTORY)
WHERE   START_TIME BETWEEN $TS_START AND $TS_END    
AND		WAREHOUSE_NAME = $WAREHOUSE_NAME  
