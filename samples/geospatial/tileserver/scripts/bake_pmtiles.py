#!/usr/bin/env python3
"""Bake public.features_mvt into a PMTiles archive (arm 2).

Uses a single, reused psycopg2 connection (with reconnect-on-failure) to be
gentle on the small Postgres instance's connection budget. Writes tiles z0..MAXZOOM
directly to a .pmtiles file. Tiles are gzipped; header marks GZIP so MapLibre /
Martin decompress transparently.
"""
import gzip
import os
import sys
...
def connect():
    ...
    for attempt in range(6):
        try:
            c = psycopg2.connect(service="tileserver", connect_timeout=15)
            c.autocommit = True
            return c
