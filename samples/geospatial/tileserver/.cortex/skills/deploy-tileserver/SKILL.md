...
- Container runtime (Docker or Podman); Python 3 with `snowflake-connector-python`, `psycopg2`, and `pmtiles`; Snowflake CLI (`snow`).
...
- The source dataset available in core Snowflake. Default is the free **Overture Maps - Divisions** Marketplace share (`OVERTURE_MAPS__DIVISIONS.CARTO.DIVISION_AREA`); acquire it from the Marketplace or point `--source-table` at your own table.
...
3. **Data** - `scripts/sync_to_pg.py` loads the source into `public.features` with a GiST index.
...
| `--source-table <fqn>` | `OVERTURE_MAPS__DIVISIONS.CARTO.DIVISION_AREA` | Snowflake source table |
| `--country <cc>` | (all) | Narrow the dataset (e.g. `US`) for a small/fast bake |
