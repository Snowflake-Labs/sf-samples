...
For the recommended agent-driven workflow, see the [sample README](../README.md).
This document describes the advanced CLI path. Run commands from the sample root,
not from this `docs/` directory.
...
still require acceptance testing. See the [validation record](validation.md).
...
in [docs/source-contract.md](source-contract.md). This is an Overture adapter,
...
See [docs/validation.md](validation.md) for checks completed during sample
...
| Preflight rejects a column or geometry | Compare your source with the [source contract](source-contract.md). The adapter requires native GEOGRAPHY and the supported Overture nested layout. |
