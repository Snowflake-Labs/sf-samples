--I thought of adding tag_name (cost_center) as dynamic, but chose to hardcode it for specificity and simplicity in calling code.
...
$$
SELECT coalesce(
    IFF(QUERY_TAG <> '',try_parse_json(QUERY_TAG):"<% ctx.env.finops_tag_name %>"::varchar, NULL)
