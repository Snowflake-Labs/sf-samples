...
(select (date(date_trunc($time_slice,start_time))) time_period from SNOWFLAKE.ORGANIZATION_USAGE.WAREHOUSE_METERING_HISTORY
...
CREATE OR REPLACE TRANSIENT TABLE ORG_UNIT_ECON_ACCOUNTS AS
with  wh_metering as (
select 
         account_name
...
where start_time > $start_rpt AND start_time < $end_rpt --start 90 days back on the 1st day of the week to avoid middle of the week
group by all
),
warehouse_load as (
...
order by 1),
combined as (
  select  wmet.account_name,wmet.time_period, active_users,sum(wmet.credits_used) credits_used_calc ,
  ...
  ON (wmet.account_name = wem.account_name
     and wmet.time_period = wem.time_period)
     group by all
),
warehouse_load_groupby as (
select account_name,time_period,sum(num_whs_high_concurrency) num_whs_high_concurrency,sum(num_whs_low_concurrency) num_whs_low_concurrency from warehouse_load
group by all
),
all_results as (
select * from (select account_name,'NUM WHS WITH LOW CONCURRENCY' as metric,time_period,num_whs_low_concurrency from warehouse_load_groupby) 
    pivot(sum(num_whs_low_concurrency) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'NUM WHS WITH HIGH CONCURRENCY' as metric,time_period,num_whs_high_concurrency from warehouse_load_groupby) 
    pivot(sum(num_whs_high_concurrency) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'COMPUTE CREDITS' as metric,time_period,credits_used from wh_metering)
         pivot(sum(credits_used) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'ACTIVE USERS' as metric,time_period,active_users from wh_economic_metrics) 
    pivot(sum(active_users) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'EFFICIENCY SCORE (CREDITS/1000 QRYS)' as metric,time_period,credits_per_thousand_queries from combined) 
    pivot(sum(credits_per_thousand_queries) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'NUM QUERIES EXECUTED' as metric,time_period,query_count from wh_economic_metrics) 
    pivot(sum(query_count) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'GB SCANNED' as metric,time_period,gb_scanned from wh_economic_metrics) 
    pivot(sum(gb_scanned) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'PCT QUERY HRS RUNNING HIGHLY DISTRIB  ' as metric,time_period,pct_wh_highly_utilized from combined) 
    pivot(sum(pct_wh_highly_utilized) for time_period in (ANY order by time_period))
UNION ALL
select * from (select account_name,'QUERY RUNTIME DURATION (HRS)' as metric,time_period,execution_time_hrs from wh_economic_metrics) 
    pivot(sum(execution_time_hrs) for time_period in (ANY order by time_period))
)
select 
*,
    identifier($cols2_last_pos)-$3                                               as "BEGIN/END DELTA",
    round(100 * div0null((identifier($cols2_last_pos)-$3),$3),1)                 as "BEGIN/END % CHANGE",
from all_results;