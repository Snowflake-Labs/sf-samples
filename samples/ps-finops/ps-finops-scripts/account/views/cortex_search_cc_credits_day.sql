...
SELECT
   DATE_TRUNC('DAY', SFQ.USAGE_DATE) AS DAY,
...
FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_SEARCH_DAILY_USAGE_HISTORY SFQ
--Schema
LEFT JOIN SNOWFLAKE.ACCOUNT_USAGE.TAG_REFERENCES ts ON ts.object_database = sfq.database_name
  AND ts.object_name = sfq.schema_name
