...
CREATE OR REPLACE VIEW COMPUTE_AND_QAS_CC_AVG_PER_QUERY_CREDITS_DAY COMMENT = 'Title: Compute & Query Acceleration Service (QAS) Credits Average Per Query and Cost Center by Day. Description: Analyze the major costs incurred in Snowflake Compute use with average cost per query for each cost center. Does not account for extra cloud services fees above the 10% threshold.'
AS
...
SELECT
   ...
   , ROUND(COMPUTE_CREDITS/QUERY_COUNT,2) AS COST_PER_QUERY_DAILY_AVG
