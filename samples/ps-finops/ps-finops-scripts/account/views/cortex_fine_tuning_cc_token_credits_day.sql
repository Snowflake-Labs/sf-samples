...
SELECT
   --DATE_TRUNC('DAY', SF.ADJUSTED_START_TIME) AS DAY,  --commneted on 18th July 2025
   DATE_TRUNC('DAY', SF.START_TIME) AS DAY,
   COALESCE(TW.TAG_VALUE, MODEL_NAME) AS COST_CENTER,
   SUM(TOKEN_CREDITS) AS TOKEN_CREDITS,
FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_FINE_TUNING_USAGE_HISTORY SF
JOIN WH_ID ON SF.WAREHOUSE_ID = WH_ID.WAREHOUSE_ID
