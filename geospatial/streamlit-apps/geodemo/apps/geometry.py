...
def app():
    ...
    srid = sess.sql("SELECT MIN(ST_SRID(GEO)) AS srid FROM GEOPLAYGROUND.GEO.OBJ_OF_INTEREST").collect()
    srid = pd.DataFrame(srid).loc[0]["SRID"]
        # SELECT FEATURE, GEO FROM GEOPLAYGROUND.GEO.OBJ_OF_INTEREST
    spatialfeatures = sess.table('OBJ_OF_INTEREST').select(col("FEATURE"),col("GEO"))
    ...
    gdf = gpd.GeoDataFrame(df[['FEATURE']], geometry=gs, crs=srid).to_crs(4326)
    ...
    folium_static(map)
    st.markdown("<small>Area color is based on the FEATURE column</small>", unsafe_allow_html=True)